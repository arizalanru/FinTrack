    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(backgroundColor: Colors.green, content: Text('Dompet berhasil ditambahkan')),
        );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(backgroundColor: Colors.red, content: Text('Gagal menambahkan dompet: $e')),
        );
      }
    } finally {
      if (mounted) {
    ...
    return Scaffold(
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            const Text('Pilih Jenis Dompet', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                _buildTypeSelector('Tunai', Icons.money, 'Tunai'),
                _buildTypeSelector('Bank', Icons.account_balance, 'Bank'),
                _buildTypeSelector('E-Wallet', Icons.account_balance_wallet, 'E-Wallet'),
              ],
            ),
            const SizedBox(height: 24),
            const Text('Nama Dompet', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            TextFormField(
              controller: _nameController,
              decoration: _inputDecoration('Contoh: BCA, DANA, Tunai'),
              validator: (value) => value == null || value.isEmpty ? 'Nama dompet tidak boleh kosong' : null,
            ),
            const SizedBox(height: 24),
            const Text('Jumlah Saldo', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            TextFormField(
              ...
              validator: (value) => value == null || value.isEmpty ? 'Saldo awal tidak boleh kosong' : null,
            ),
